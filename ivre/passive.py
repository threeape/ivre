...
def parse_p0f_line(line, include_port=False, sensor=None, recontype=None):
    ...
    if line[1].startswith(b"UNKNOWN "):
        ...
        sig = line[1][line[1].index(b" Signature: ") + 12 :][1:-1].split(b":")[
            :6
        ]
        if b" (up: " in line[1]:
    ...
    spec = {
        ...
        "addr": line[0][
            line[0].index(b"> ") + 2 : line[0].index(b":")
        ].decode(),
        "distance": dist,
        "value": osname.decode(),
    ...
    return (
        datetime.fromtimestamp(float(line[0][1 : line[0].index(b">")])),
        spec,
    )
...
TCP_SERVER_PATTERNS = [
    ...
    (
        re.compile(
            ...
            b"^(.)\x00\x00" b"\x00"  # packet length  # packet number
            # https://dev.mysql.com/doc/dev/mysql-server/latest/page_protocol_connection_phase_packets_protocol_handshake_v10.html
            b"\x0a" b"([3456]\\.[-_~\\.\\+\\w]+)\x00"  # protocol  # version
...
def _prepare_rec(spec, ignorenets, neverignore):
    ...
    if (
        spec["recontype"] == "HTTP_CLIENT_HEADER"
        and spec.get("source") == "USER-AGENT"
    ):
    ...
    # host and sensor in the same records.
        ...
        if authtype.lower() == "digest":
            try:
                ...
                value = [
                    val
                    for val in _split_digest_auth(spec["value"][6:].strip())
    ...
    # SSL_{CLIENT,SERVER} JA3
        ...
        if spec["recontype"] == "SSL_SERVER":
            ...
            spec["source"] = (
                "ja3-%s" % hashlib.new("md5", clientvalue.encode()).hexdigest()
            )
    # Check DNS Blacklist answer
    elif spec["recontype"] == "DNS_ANSWER":
        if any(
            spec["value"].endswith(dnsbl)
...
_getinfos_tcp_srv_banner()"""
    return _getinfos_from_banner(
        utils.nmap_decode_data(spec["value"]) + b"\r\n"
    )
...
_GETINFOS_FUNCTIONS = {
    ...
    "HTTP_SERVER_HEADER": {"SERVER": _getinfos_http_server},
