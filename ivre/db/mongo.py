            ...
            if self.username is not None:
                if self.password is not None:
                    ...
                    self.db.authenticate(
                        self.username, mechanism=self.mechanism
                    )
                else:
                    raise TypeError(
                        "provide either 'password' or 'mechanism'"
                        " with 'username'"
                    )
            return self._db
...
e.g.  .explain()) based on the column and a filter.
        ...
        if "sort" in kargs and any(
            fld in (field for field, _ in kargs["sort"])
    ...
    def ip2internal(addr):
        ...
        return [
            ...
            for val in struct.unpack("!QQ", utils.ip2bin(addr))
        ]
    ...
    def explain(self, cursor, indent=None):
        return json.dumps(
            cursor.explain(), indent=indent, default=self.serialize
        )
...
want to do something special here, e.g., mix with other records.
    ...
    def migrate_schema(self, colnum, version):
        ...
        while version in self.schema_migrations[colnum]:
            new_version, migration_function = self.schema_migrations[colnum][
                version
            ]
            utils.LOGGER.info(
            ...
            if self.mongodb_32_more:
                ...
                for idx in new_indexes:
                    try:
                        ...
                        utils.LOGGER.debug(
                            "Cannot create index %s", idx, exc_info=True
                        )
            if new_indexes:
            ...
            for action, indexes in viewitems(
                self.schema_migrations_indexes[colnum].get(new_version, {})
            ):
                if action == "ensure":
                    continue
                function = getattr(
                    self.db[self.columns[colnum]], "%s_index" % action
                )
                for idx in indexes:
                    try:
    ...
    def _distinct(
        self, column, field, flt=None, sort=None, limit=None, skip=None
    ):
        """This method makes use of the aggregation framework to
        ...
        if is_ipfield:
            if self.mongodb_32_more:
                return (
                    None if res["_id"][0] is None else res["_id"]
                    for res in cursor
                )
            return (
                [int(val) for val in res] if res[0] else None
    ...
    def _features_port_list(
        ...
        pipeline.extend([{"$project": project}, {"$group": {"_id": "$field"}}])
        if self.mongodb_32_more:
            if not yieldall:
    ...
    def _features_port_list_pipeline(
        self, flt, use_service, use_product, use_version
    ):
    ...
    def searchhosts(cls, hosts, neg=False):
        return {
            "$and"
            if neg
            else "$or": [cls.searchhost(host, neg=neg) for host in hosts]
        }
...
class MongoDBActive(MongoDB, DBActive):
    ...
    indexes = [
        ...
        [
            ...
            (
                [("addr_0", pymongo.ASCENDING), ("addr_1", pymongo.ASCENDING)],
                {},
            ),
            ...
            3: {
                "ensure": [
                    (
                        [
                            (
                                "ports.scripts.ls.volumes.volume",
                                pymongo.ASCENDING,
                            )
                        ],
                        {"sparse": True},
...
it is not expected)."""
    ...
    def removescreenshot(self, host, port=None, protocol="tcp"):
        ...
        for p in host.get("ports", []):
            if port is None or (
                p["port"] == port and p.get("protocol") == protocol
            ):
                if "screenshot" in p:
    ...
    def get_ips_ports(self, flt, limit=None, skip=None):
        ...
        return (
            (
                dict(
                    res, addr=self.internal2ip([res["addr_0"], res["addr_1"]])
                )
                for res in cur
            ),
            count,
    ...
    def get_mean_open_ports(self, flt):
        ...
        return self.db[self.columns[self.column_hosts]].aggregate(
            aggr, cursor={}
        )
    ...
    def searchport(port, protocol="tcp", state="open", neg=False):
        ...
        if state == "open":
            return {
                "openports.%s.ports" % protocol: {"$ne": port} if neg else port
            }
        if neg:
            return {
                "$or": [
    ...
    def searchports(self, ports, protocol="tcp", state="open", neg=False):
        ...
        return {
            "ports": {
                "$all": [
                    self.searchport(
                        port, protocol=protocol, state=state, neg=neg
                    )["ports"]
                    for port in ports
                ]
            }
        ...
        if values is not None:
            ...
            if isinstance(values, (basestring, utils.REGEXP_T)):
                ...
                for field, value in viewitems(values):
                    req[
                        "%s.%s"
                        % (xmlnmap.ALIASES_TABLE_ELEMS.get(name, name), field)
                    ] = value
        if not req:
    ...
    def searchhttptitle(self, title):
        return self.searchscript(
            name={"$in": ["http-title", "html-title"]}, output=title
        )
    ...
    def searchhop(cls, hop, ttl=None, neg=False):
        ...
        if ttl is None:
            flt = {
                "traces.hops": {
                    "$elemMatch": {"ipaddr_0": hop[0], "ipaddr_1": hop[1]}
                }
            }
    ...
    def topvalues(
        ...
        if field == "category":
            ...
            if self.mongodb_32_more:
                specialproj = {
                    "_id": 0,
                    "city": ["$infos.country_code", "$infos.city"],
                }
                ...
                specialproj = {
                    "_id": 0,
                    "city": _old_array("$infos.country_code", "$infos.city"),
                }
            ...
            if self.mongodb_32_more:
                ...
                def outputproc(x):
                    return {
                        ...
                        "_id": tuple(
                            int(y) if i == 0 else y
                            for i, y in enumerate(x["_id"].split("###"))
                        ),
                    }

        elif field == "addr":
            specialproj = {"_id": 0, "$addr_0": 1, "$addr_1": 1}
            if self.mongodb_32_more:
                ...
                specialflt = [
                    {
                        "$project": {
                            field: _old_array(
                                "$addr_0", "$addr_1", convert_to_string=True
                            )
                        }
                    }
                ]
                ...
                def outputproc(x):
                    return {
                        ...
                        "_id": self.internal2ip(
                            [int(val) for val in x["_id"].split("###")]
                        ),
                    }
            ...
            def outputproc(x):
                return {
                    "count": x["count"],
                    "_id": "%s/%d"
                    % (utils.int2ip(int(x["_id"]) * 2 ** (32 - mask)), mask),
                }
            ...
            if self.mongodb_32_more:
                ...
                specialflt = [
                    {"$match": {flt_field: info}},
                    {
                        "$project": {
                            field: _old_array(
                                "$ports.protocol", {"$toLower": "$ports.port"}
                            )
                        }
                    },
                ]
            ...
            if self.mongodb_32_more:
                ...
                def outputproc(x):
                    return {
                        "count": x["count"],
                        "_id": tuple(
                            elt if elt else None
                            for elt in x["_id"].split("###")
                        ),
                    }
            ...
            if service.isdigit():
                ...
                flt = self.flt_and(
                    flt, self.searchproduct(product, service=service)
                )
                specialflt = [
                    {
                        "$match": {
            ...
            if field == "useragent":
                ...
                flt = self.flt_and(
                    flt, self.searchuseragent(useragent=subfield)
                )
                specialflt = [
            ...
            flt = self.flt_and(
                ...
                self.searchja3server(
                    value_or_hash=value1, client_value_or_hash=value2
                ),
            )
            specialproj = {
                "_id": 0,
            ...
            if self.mongodb_32_more:
                ...
                def outputproc(x):
                    return {
                        "count": x["count"],
                        "_id": tuple(
                            null_if_empty(val) for val in x["_id"].split("###")
                        ),
                    }
            ...
            flt = self.flt_and(
                flt,
                self.searchscript(
                    name="ike-info", values={"transforms": {"$exists": True}}
                ),
            )
            specialproj = {
            ...
            specialflt = [
                {
                    "$match": {
                        "ports.scripts.http-headers.name": field[8:].lower()
                    }
                }
            ]
            ...
            flt = self.flt_and(
                flt, self.searchscript(name="mongodb-databases")
            )
            ...
            field.startswith("file") and field[4] in ".:"
        ):
            ...
            if scripts is not None:
                ...
                specialflt = [
                    {
                        "$match": {
                            "ports.scripts.id": flt["ports.scripts"][
                                "$elemMatch"
                            ]["id"]
                        }
                    },
        ...
        cursor = self.set_limits(
            self.db[self.columns[self.column_hosts]].aggregate(
                pipeline, cursor={}
            )
        )
    ...
    def diff_categories(
        self, category1, category2, flt=None, include_both_open=True
    ):
        ...
        cursor = (
            dict(x["_id"], value=categories_to_val(x["categories"]))
            for x in cursor
        )
...
class MongoDBNmap(MongoDBActive, DBNmap):
    ...
    def __init__(
        ...
        **kwargs
        ...
        MongoDBActive.__init__(
            self, host, dbname, colname_hosts=colname_hosts, **kwargs
        )
    ...
    def is_scan_present(self, scanid):
        if (
            self.find_one(
                self.columns[self.column_scans], {"_id": scanid}, fields=[]
            )
            is not None
        ):
            return True
    ...
    def remove(self, host):
        ...
        for scanid in self.getscanids(host):
            if (
                self.find_one(
                    self.columns[self.column_hosts], {"scanid": scanid}
                )
                is None
            ):
...
class MongoDBPassive(MongoDB, DBPassive):
    ...
    def __init__(self, host, dbname, colname_passive="passive", **kargs):
        MongoDB.__init__(self, host, dbname, **kargs)
...
want to do something special here, e.g., mix with other records.
        ...
        if colname == self.columns[self.column_passive]:  # just in case
            ...
            return super(MongoDBPassive, self)._migrate_update_record(
                colname, recid, update
            )
...
e.g.  .explain()).
        ...
        return self._get_cursor(
            self.columns[self.column_passive], flt, **kargs
        )
...
returned to backend-agnostic functions.
        ...
        """
        try:
            rec["addr"] = cls.internal2ip(
                [rec.pop("addr_0"), rec.pop("addr_1")]
            )
        except (KeyError, socket.error):
            pass
...
"""
    ...
    def _fix_sizes(cls, spec):
        ...
        for field in list(spec.get("infos", {})):
            ...
            if (
                ...
                and len(value) > utils.MAXVALLEN // 10
            ):
    ...
    def topvalues(self, field, flt=None, distinct=True, **kargs):
        ...
        if field == "addr":
            if self.mongodb_32_more:
                specialproj = {"_id": 0, "addr": ["$addr_0", "$addr_1"]}
        ...
        pipeline = self._topvalues(
            field, flt=flt, specialproj=specialproj, **kargs
        )
        cursor = self.set_limits(
    ...
    def _features_port_get(
        ...
        n_features = len(features)
        for rec in self.get(
            self.flt_and(flt, {"port": {"$exists": True}}), sort=[("addr", 1)]
        ):
    ...
    def searchuseragent(useragent=None, neg=False):
        ...
        if useragent is None:
            return {"recontype": "HTTP_CLIENT_HEADER", "source": "USER-AGENT"}
        return {
    ...
    def searchcert(keytype=None):
        if keytype is None:
            return {"recontype": "SSL_SERVER", "source": "cert"}
        return {
    ...
    def searchja3client(cls, value_or_hash=None):
        return dict(
            cls._searchja3(value_or_hash), recontype="SSL_CLIENT", source="ja3"
        )
    ...
    def searchnewer(timestamp, neg=False, new=False):
        ...
        return {
            "lastseen"
            if new
            else "firstseen": {"$lte" if neg else "$gt": timestamp}
        }
...
class MongoDBAgent(MongoDB, DBAgent):
    ...
    def get_agent(self, agentid):
        return self.find_one(
            self.columns[self.column_agents], {"_id": agentid}
        )
    ...
    def get_scan(self, scanid):
        ...
        if "target_info" not in scan:
            ...
            if target is not None:
                ...
                self.db[self.columns[self.column_scans]].update(
                    {"_id": scanid}, {"$set": {"target_info": target_info}}
                )
...
scan object on success, and raises a LockError on failure.
    ...
    def get_master(self, masterid):
        return self.find_one(
            self.columns[self.column_masters], {"_id": masterid}
        )
