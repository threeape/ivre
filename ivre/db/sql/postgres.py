                    ...
                    try:
                        ...
                        self.db.execute(
                            ...
                            .values(
                                data={
                                    "ssl-cert": dict(
                                        rec.data["ssl-cert"], type=pubkeytype
                                    )
                                }
                            )
                        )
    ...
    def topvalues(
        ...
        if field == "port":
            ...
            if info.isdigit():
                ...
                flt = self.flt_and(
                    flt, self.searchport(info[1], protocol=info[0])
                )
                field = self._topstructure(
                    self.tables.port,
                    [
                ...
                field = self._topstructure(
                    ...
                    and_(
                        ...
                        self._searchstring_re(
                            column("http_user_agent").op("->>")(0), subfield
                        ),
                    ),
                    None,
                    func.jsonb_array_elements(
            ...
            if value is None:
                ...
                field = self._topstructure(
                    ...
                    and_(
                        self.tables.script.name == "ssl-ja3-client",
                        self._searchstring_re(
                            column("ssl_ja3_client").op("->>")(subkey), value
                        ),
                    ),
                    None,
                    func.jsonb_array_elements(
            ...
            if value1 is not None:
                condition = and_(
                    condition,
                    self._searchstring_re(
                        column("ssl_ja3_server").op("->>")(subkey1), value1
                    ),
                )
            ...
            field = self._topstructure(
                self.tables.scan, [self.tables.scan.source]
            )
        elif field == "domains":
            field = self._topstructure(
            ...
            base1 = (
                select(
                    [
                        func.unnest(self.tables.hostname.domains).label(
                            "domains"
                        )
                    ]
                )
        ...
        if field.base == self.tables.scan:
            ...
            req = select(
                [func.count().label("count")] + field.fields
        ...
        if outputproc is None:
            return (
                ...
                for result in self.db.execute(
                    req.order_by(order).limit(topnbr)
                )
            )
        else:
            return (
            ...
            .on_conflict_do_update(
                ...
                set_={
                    ...
                    "time_start": func.least(
                        self.tables.scan.time_start, insrt.excluded.time_start
                    ),
                    "time_stop": func.greatest(
                        self.tables.scan.time_stop, insrt.excluded.time_stop
                    ),
                },
            )
...
class PostgresDBPassive(PostgresDB, SQLDBPassive):
    ...
    def _insert_or_update(self, timestamp, values, lastseen=None):
        ...
        upsert = {
            "firstseen": func.least(self.tables.passive.firstseen, timestamp),
            "lastseen": func.greatest(
                self.tables.passive.lastseen, lastseen or timestamp
            ),
            ...
            flt.query(
                select(
                    [
                        self.tables.passive.addr,
                        func.array_agg(
                            func.distinct(postgresql.array(fields))
                        ),
                    ]
