...
def get_notepad_pages_localdokuwiki(pagesdir="/var/lib/dokuwiki/data/pages"):
    ...
    return [
        page[:-4] for page in os.listdir(pagesdir) if ipaddr_page.match(page)
    ]
...
def flt_from_query(dbase, query, base_flt=None):
    ...
    for (neg, param, value) in query:
        if not neg and param == "skip":
            ...
            flt = dbase.flt_and(
                flt,
                dbase.searchobjectid(
                    value.replace("-", ",").split(","), neg=neg
                ),
            )
        elif param == "host":
            ...
            flt = dbase.flt_and(
                flt,
                dbase.searchcountry(utils.str2list(value.upper()), neg=neg),
            )
        elif param == "city":
            flt = dbase.flt_and(
            ...
            if value and value[-1].isalpha():
                unit = {"s": 1, "m": 60, "h": 3600, "d": 86400, "y": 31557600}[
                    value[-1]
                ]
            ...
            if ":" in value:
                ...
                port = int(port)
                flt = dbase.flt_and(
                    flt, dbase.searchservice(utils.str2regexp(req), port=port)
                )
            else:
                flt = dbase.flt_and(
                    flt, dbase.searchservice(utils.str2regexp(value))
                )
            ...
            if len(value) == 1:
                flt = dbase.flt_and(
                    flt,
                    dbase.searchscript(
                        name=utils.str2regexp(value[0]), neg=neg
                    ),
                )
            else:
                flt = dbase.flt_and(
        ...
        # results of scripts or version scans
            ...
            flt = dbase.flt_and(
                flt, dbase.searchbanner(utils.str2regexp(value))
            )
        elif param == "cookie":
            ...
            try:
                ...
                status = None
            flt = dbase.flt_and(
                flt, dbase.searchvuln(vulnid=vulnid, status=status)
            )
        elif not neg and param == "geovision":
            flt = dbase.flt_and(flt, dbase.searchgeovision())
        elif param == "httptitle":
            flt = dbase.flt_and(
                flt, dbase.searchhttptitle(utils.str2regexp(value))
            )
            ...
            if subfield in ["fingerprint", "key", "type", "bits"]:
                ...
                flt = dbase.flt_and(
                    flt, dbase.searchsshkey(**{subfield: value})
                )
            else:
            ...
            if value is None:
                ...
                flt = dbase.flt_and(
                    flt, dbase.searchhttphdr(name=utils.str2regexp(value))
                )
            ...
            flt = dbase.flt_and(
                ...
                dbase.searchja3client(
                    value_or_hash=(
                        None if value is None else utils.str2regexp(value)
                    ),
                    neg=neg,
                ),
            )
        ...
        # traceroute
            ...
            if ":" in value:
                ...
                flt = dbase.flt_and(flt, db.view.searchhop(value, neg=neg))
        elif param == "hopname":
        ...
        # sort
            ...
            if (
                ...
                flt = dbase.flt_and(
                    flt,
                    dbase.searchports(
                        [int(val) for val in param.split(",")], neg=neg
                    ),
                )
                ...
                if param[1] and param[1][0] == "=":
                    ...
                    flt = dbase.flt_and(
                        flt,
                        dbase.searchcmp(
                            param[0], int(param[1]), ">=" if neg else "<"
                        ),
                    )
            elif ">" in param:
                param = param.split(">", 1)
