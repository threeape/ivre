...
def get_nmap_action(subdb, action):
    ...
    if action == "timeline":
        ...
        if request.params.get("modulo") is None:
            ...
            def r2time(r):
                return int(utils.datetime2timestamp(r["starttime"])) % int(
        ...
        if flt_params.ipsasnumbers:
            for res in output:
                result.setdefault(res["addr"], []).append(
                    [res["port"], res["value"]]
                )
                count += 1
        else:
...
def get_nmap_top(subdb, field):
    ...
    # hack to avoid a trailing comma
    cursor = iter(
        subdb.topvalues(field, flt=flt_params.flt, least=least, topnbr=topnbr)
    )
    try:
        rec = next(cursor)
    except StopIteration:
...
def import_files(subdb, source, categories, files):
    ...
    for fileelt in files:
        ...
        try:
            if subdb.store_scan(
                fdesc.name, categories=categories, source=source
            ):
                count += 1
            ...
            utils.LOGGER.warning(
                "Could not import %s", fdesc.name, exc_info=True
            )
    return count
...
def post_nmap(subdb):
    ...
    count = import_files(
        db.view if subdb == "view" else db.nmap, source, categories, files
    )
