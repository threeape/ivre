...
DOI = Values(
    {0: "ISAKMP", 1: "IPSEC", 2: "GDOI"}  # RFC2408  # RFC2407  # RFC3547
)
...
TRANSFORM_VALUES = {
    ...
    5: ("GroupType", Values({1: "MODP", 2: "ECP", 3: "EC2N"})),
    ...
    11: ("LifeType", Values({1: "Seconds", 2: "Kilobytes"})),
        ...
        while data:
            ...
            if transf_type & 0x8000:
                ...
                for val in data[:value_length]:
                    value = value * 256 + (
                        val if isinstance(val, int) else ord(val)
                    )
            try:
        ...
        if "transforms" in output:
            ...
            for tr in output["transforms"]:
                txtoutput.append(
                    ...
                    % ", ".join(
                        "%s: %s" % (key, value)
                        for key, value in sorted(viewitems(tr))
                    )
                )
