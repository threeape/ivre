...
def main():
    parser, use_argparse = ivre.utils.create_argparser(
        __doc__, extraargs="scan"
    )
    if use_argparse:
        parser.add_argument(
            "scan", nargs="*", metavar="SCAN", help="Scan results"
        )
    parser.add_argument(
        "-c", "--categories", default="", help="Scan categories."
    )
    ...
    for scan in scans:
        try:
            ...
            ivre.utils.LOGGER.warning(
                "Exception (file %r)", scan, exc_info=True
            )
